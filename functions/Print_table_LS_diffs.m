function Print_table_LS_diffs(VAR_size,h_list)

addpath([pwd,'/functions']);

% Prelimns
n_vars_to_display = 7;
models_to_print   = [{'DFM'},{'FAVAR1'},{'BVAR MINN'},{'BCTRVAR (opt 3)'},{'BCTRVAR (opt 3 cov)'}];
...
f_out_1 = [this_out,'Table_LSD_',VAR_size,'_pan_a.out'];
f_out_2 = [this_out,'Table_LSD_',VAR_size,'_pan_b.out'];
f_out_3 = [this_out,'Table_LSD_',VAR_size,'_pan_c.out'];
...
[data,textdata,celldata] = xlsread([pwd,'/Output/Table_MSFE_',VAR_size,'(vs VAR).xlsx'],'MSFE');
[data_DM,~,~]            = xlsread([pwd,'/Output/Table_MSFE_',VAR_size,'(vs VAR).xlsx'],'DM tests');
