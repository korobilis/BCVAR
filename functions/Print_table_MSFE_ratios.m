function Print_table_MSFE_ratios(VAR_size,h_list,varargin)
...
if nargin > 2
    f_out_1 = [this_out,'Table_',VAR_size,'_',datestr(date_start,'yyyy'),'_',datestr(date_end,'yyyy'),'_pan_a.out'];
    f_out_2 = [this_out,'Table_',VAR_size,'_',datestr(date_start,'yyyy'),'_',datestr(date_end,'yyyy'),'_pan_b.out'];
    f_out_3 = [this_out,'Table_',VAR_size,'_',datestr(date_start,'yyyy'),'_',datestr(date_end,'yyyy'),'_pan_c.out'];
else
    f_out_1 = [this_out,'Table_',VAR_size,'_pan_a.out'];
    f_out_2 = [this_out,'Table_',VAR_size,'_pan_b.out'];
    f_out_3 = [this_out,'Table_',VAR_size,'_pan_c.out'];
end
...
for h=1:length(h_cols)
    ...
    for m=models_to_print
        ...
        this_row_indx = this_row_indx(1:end-2); % remove the WTMSFE results from this table
        if isequal(m,{'DFM'})
