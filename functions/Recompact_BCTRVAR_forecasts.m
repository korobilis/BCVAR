function Recompact_BCTRVAR_forecasts(VAR_size,RP_type,n_psi,stdata,apply_bcr,weight_scheme,cov_comp)
...
for irep = T_thres:T-h
    ...
    f_id = [pwd,'/Temp/',sprintf('%s_%s_%g_%g_%g_%g_%g_%g_%s.mat','BCTRVAR',VAR_size,RP_type,n_psi,stdata,apply_bcr,weight_scheme,cov_comp,irep_to_print)];
    load(f_id);
...
%% Save output
save([pwd,'/Output/',sprintf('%s_%s_%g_%g_%g_%g_%g_%g.mat','BCTRVAR',VAR_size,RP_type,n_psi,stdata,apply_bcr,weight_scheme,cov_comp)],'Y','fore*','msfe*','PL*','-mat');
