function [Y_f_BMA] = BCTRVAR_TVP(Y,p,constant,h,ndraws,RP_type,n_psi,stdata,apply_bcr,weight_scheme,cov_comp,sparsity,series_to_eval,lambda,delta,forgetting)                   
...
prior_c = 0.5;
beta_0_prmean = zeros(n_m1+2*M,1);
beta_0_prvar = prior_c*eye(n_m1+2*M,n_m1+2*M);
Sigma_0  = 0.1*cov(Y);
...
for psi_l = 1:n_psi
    ...
    for irep = 1:M
        ...
        n_vars = size(X_new,1);                          % Number of variables in eq irep. This changes in each eq due to the triangular structure
        PX = X_new; XP = PX'; PXXP = PX*XP;              % Define these here once, we need them several times        
        
        [beta_t,SIGMA_t,lams,dels] = FF_Kalman_filter(Y(:,irep),XP,1,n_vars,T,lambda,delta,forgetting,beta_0_prmean(1:n_vars),beta_0_prvar(1:n_vars,1:n_vars),Sigma_0(irep,irep));
        
        A_post{irep,1} = beta_t(:,end);
        sigma(irep,1) = SIGMA_t(:,:,end);
        
        % save lambda_t and delta_t
        lam_save{irep,1} = lams;
        del_save{irep,1} = dels;
        
        % equation-specific BIC
        if irep < max(series_to_eval)+1
            BIC2(Model_no,irep) = size(X_comp{irep},2)*log(T) + T*log(sigma(irep,1));        
        end
    ...
    while max(abs(eig(By))) > 1
        ...
        for irep = 1:M
            ...
            A_post{irep,1} = beta_t(:,end);
            sigma(irep,1) = SIGMA_t(:,:,end);                                     % Mean estimate of sigma in eq irep
    ...
    BIC(Model_no,1) = log(det(err_post'*err_post/T)) + (log(T)/T)*numel(beta_red_VAR);  
end
